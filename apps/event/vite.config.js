...
const generateBuildInputOption = (root) => {
  ...
  const template = fs.readFileSync(templatePath, 'utf-8');
  console.log("setting",Object.keys(setting))
  setting[setting.key].forEach(item => {
    const key = item.key;
    const title = item.title;
    
    let modifiedTemplate = template
      .replace('<title></title>', `<title>${title}${setting.suffix}</title>`)
      .replace('<meta name="description" content="">', `<meta name="description" content="${setting.desc}">`)
      .replace('<meta name="keywords" content="">', `<meta name="keywords" content="${setting.keys}">`)
      .replace('<script type="module" src="../main.js"></script>',`<script type="module" src="../../pages/${item.key}/index.js"></script>` )

    const outputPath = path.join(outputDir, key, 'index.html');
    fs.mkdirSync(path.dirname(outputPath), { recursive: true });
    fs.writeFileSync(outputPath, modifiedTemplate);
    input[item.key] = resolve('', outputPath)
  });
console.log(input)
  return input;
}
